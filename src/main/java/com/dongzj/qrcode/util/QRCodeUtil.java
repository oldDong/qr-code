package com.dongzj.qrcode.util;
...
public final class QRCodeUtil extends LuminanceSource {
    ...
    public QRCodeUtil(BufferedImage image, int left, int top, int width, int height) {
        ...
        int sourceHight = image.getHeight();
        if (left + width > sourceWidth || top + height > sourceHight) {
        ...
        this.image = new BufferedImage(sourceWidth, sourceHight, BufferedImage.TYPE_BYTE_GRAY);
     ...
     * @param height   高
     * @param format   图片格式
     * @param pathName 生成的二维码图片路径
     * @return pathName 生成路径
     */
    ...
    public static String parseQRCode(String filePath) {
        ...
        try {
            ...
            logger.error("parseQRCode exception: {}", e.getMessage());
        }
        return content;
    }

    public static void main(String[] args) {
        String text = "车主电话：18868822800";
        int width = 500;
        int height = 500;
        String format = "png";
        String filePath = "/Users/dongzj/Workspaces/test/qr-code/src/main/resources/qrcode/dongzj.png";
        try {
            String pathName = generateQRCode(text, width, height, format, filePath);
